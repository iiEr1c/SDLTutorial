...
# LESSON 25 [Cappint Frame Rate]

教程只介绍了如果运行的过快需要```SDL_Delay```, 但是如果过慢呢? 就行我们平常玩游戏时一样, 或许应该需要更加精确的计时方式?

# LESSON 26 [Motion]

这一LESSON主要是交会我们如何渲染移动的物体.

按照每一帧(frame)来渲染动画. 教程里提到其实游戏里的速度不是按照帧来计算的, 而是按照时间(秒)来计算. 按照帧来计算, 性能能好的计算机和性能差的计算机的移动速度是不一致的, 不可能强制要求所有的计算机以相同的帧率运行, 在多人对战中, 则需要使用共同的单位来计算, 但是在显示时, 则需要将速度转化为与当前电脑匹配的帧率速度(表述应该不太对).
